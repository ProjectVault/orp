...
extern const uint8_t bulkencryption_endpoint[32];
        ...
        uint16_t *port, const uint8_t* data)
{
    ...
    *task_fn = NULL;
    ...
    if (msel_memcmp(bulkencryption_endpoint, data, ENDPT_HASH_SIZE) == 0) {
        *endpoint = bulkencryption_endpoint;
        *task_fn = bulkencryption_task;
        return;
    }
}
