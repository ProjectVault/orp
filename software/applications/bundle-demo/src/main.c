...
extern const uint8_t encryptor_endpoint[32];
...
extern const uint8_t urchat_endpoint[32];
*/
        ...
        uint16_t *port, const uint8_t* data)
{
    ...
    *task_fn = NULL;
    ...
    if (msel_memcmp(encryptor_endpoint, data, ENDPT_HASH_SIZE) == 0) {
        *endpoint = encryptor_endpoint;
        *task_fn = encryptor_task;
        return;
    }
...
/*
    if (msel_memcmp(urchat_endpoint, data, ENDPT_HASH_SIZE) == 0) {
        *endpoint = urchat_endpoint;
        *task_fn = urchat_task;
        return;
    }
*/
